...
# ---------------------------------------------------------------------------
# Ground-truth verification helper (anti-hallucination)
# ---------------------------------------------------------------------------


def db_row_count(db: psycopg2.extensions.connection, table: str, where: str = "1=1") -> int:
    """Return row count for a table with optional WHERE clause.

    Use before/after a mutation step to verify writes actually landed.
    Example:
        before = db_row_count(db, "test_occurrences", "supplier_slug='acme'")
        # ... run mutation ...
        after = db_row_count(db, "test_occurrences", "supplier_slug='acme'")
        assert after == before + 1, "Expected exactly 1 new row"
    """
    cur = db.cursor()
    cur.execute(f"SELECT COUNT(*) FROM {table} WHERE {where}")  # noqa: S608
    return cur.fetchone()[0]


def s3_key_exists(s3_client, bucket: str, prefix: str, substring: str) -> bool:
    """Check if any S3 key under prefix contains substring.

    Use after a signal-write step to verify the signal file was actually created.
    """
